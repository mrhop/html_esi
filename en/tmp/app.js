...
;( function( $, window, undefined ) {
	...
	*/
	...
	$.Slitslider.defaults = {
		...
		width:null,
		// callbacks
		onBeforeChange : function( slide, idx,dir ) { return false; },
	...
	$.Slitslider.prototype = {
		...
		_getSize : function() {
			var width = this.options.width==null?this.$elWrapper.outerWidth( true ):this.options.width;
			this.size = {
				width : width,
		...
		_navigate : function( dir, pos ) {
			...
			this.options.onBeforeChange( $currentSlide, this.current,dir );
			...
			config.slice1opacity = configData.slice1opacity || 0;
			config.slice2opacity = configData.slice2opacity || 0;
			...
			}else if(config.orientation === 'fade'){
				slice1Style ={
					'opacity':config.slice1opacity
				};
			}
			var slice2Style="";
			if(config.orientation === 'horizontal'){
				...
				slice2Style ={
					'opacity':config.slice2opacity
				};
			}
			
			if( this.options.optOpacity&&(config.orientation === 'horizontal'||config.orientation === 'vertical') ) {
		...
		_validateValues : function( config ) {
			...
			if( config.orientation !== 'vertical' && config.orientation !== 'horizontal'&& config.orientation !== 'fade') {
